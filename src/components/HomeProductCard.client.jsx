      ...
      <form className="mt-auto">
        {options.map(({ name, values }) => {
          ...
          return values.length == 1 ? null : (
            <ProductGridOptions key={values} name={name} values={values} />
          );
        })}
  ...
  return name != "Color" ? (
    ...
    >
      {values.map(function (value , i) {
        ...
        return (
          <option key={i} value={value} defaultValue={selectedOptions[name]} id={id}>
            {value}
          </option>
        );
      })}
    ...
    values.map((value , i) => {
      ...
      const id = `option-${name}-${value}`;
      return (
        <label key={i} htmlFor={id} className="inline-block mx-3 my-2">
          <input
